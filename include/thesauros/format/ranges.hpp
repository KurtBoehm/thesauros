...
template<typename T>
struct fmt::formatter<thes::ranges::IotaRange<T>> : fmt::nested_formatter<T> {
  auto format(const thes::ranges::IotaRange<T> range, format_context& ctx) const {
...
template<typename T>
struct fmt::range_format_kind<thes::ranges::IotaRange<T>, char> {
