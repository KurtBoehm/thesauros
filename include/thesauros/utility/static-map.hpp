...
requires(TypeSequence<typename TPairs::Key...>::is_unique &&
         AutoValueSequence<TPairs::key...>::all_different)
struct StaticMap<TPairs...> {
  ...
  static constexpr bool contains = [] {
    auto impl = []<std::size_t tIdx>(StaticValue<tIdx> /*tag*/, auto rec) {
      if constexpr (tIdx == sizeof...(TPairs)) {
        return false;
      } else if constexpr (std::tuple_element_t<tIdx, Tuple>::key == tKey) {
        return true;
      } else {
        return rec(static_value<tIdx + 1>, rec);
      }
    };
...
private:
  ...
  static constexpr auto& get_impl(auto& self) {
    auto impl = [&self]<std::size_t tIdx>(StaticValue<tIdx> /*tag*/, auto rec) -> const auto& {
