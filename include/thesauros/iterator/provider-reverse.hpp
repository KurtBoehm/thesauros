...
struct Reverse {
  ...
  requires iterator_provider::RevDeref<TDerived, TForwardProvider> or
           (iterator_provider::Deref<TDerived, TForwardProvider> and
            iterator_provider::Decr<TDerived, TForwardProvider>)
  {
    if constexpr (iterator_provider::RevDeref<TDerived, TForwardProvider>) {
  ...
  requires iterator_provider::Decr<TDerived, TForwardProvider>
  {
    TForwardProvider::decr(self);
  }
  static void decr(auto& self)
  requires iterator_provider::Incr<TDerived, TForwardProvider>
  {
    TForwardProvider::incr(self);
  }
  static void iadd(auto& self, Diff diff)
  requires iterator_provider::InPlaceAdd<TDerived, TForwardProvider>
  {
    if constexpr (iterator_provider::InPlaceSub<TDerived, TForwardProvider>) {
