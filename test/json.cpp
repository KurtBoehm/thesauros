...
THES_CREATE_TYPE(SNAKE_CASE(Test3), NORMAL_CONSTRUCTOR, LAYOUT_INFO(false),
...
int main() {
  ...
  THES_ALWAYS_ASSERT(
    test::string_eq("3.141592653589793", thes::numeric_string(std::numbers::pi).value()));
  THES_ALWAYS_ASSERT(test::string_eq(
    "3.4028235e+38", thes::numeric_string(std::numeric_limits<f32>::max()).value()));
  THES_ALWAYS_ASSERT(test::string_eq(
    "2.2250738585072014e-308", thes::numeric_string(std::numeric_limits<f64>::min()).value()));
  THES_ALWAYS_ASSERT(
  ...
  THES_ALWAYS_ASSERT(test::string_eq("abc\\\"\\r\\n", thes::escaped_string("abc\"\r\n")));
  THES_ALWAYS_ASSERT(test::string_eq("\\u0007", thes::escaped_string("\a")));

  // thes::json_print

  THES_ALWAYS_ASSERT(test::string_eq("3.141592653589793", thes::json_print(std::numbers::pi)));
  THES_ALWAYS_ASSERT(
    test::string_eq("3.4028235e+38", thes::json_print(std::numeric_limits<f32>::max())));
  THES_ALWAYS_ASSERT(
    test::string_eq("2.2250738585072014e-308", thes::json_print(std::numeric_limits<f64>::min())));
  THES_ALWAYS_ASSERT(test::string_eq("-9223372036854775808",
                                     thes::json_print(std::numeric_limits<long long>::lowest())));
  THES_ALWAYS_ASSERT(test::string_eq(
    "18446744073709551615", thes::json_print(std::numeric_limits<unsigned long long>::max())));

  THES_ALWAYS_ASSERT(test::string_eq("\"abc\\\"\\r\\n\"", thes::json_print("abc\"\r\n")));
  THES_ALWAYS_ASSERT(test::string_eq("\"\\u0007\"", thes::json_print("\a")));
  ...
  THES_ALWAYS_ASSERT(test::string_eq("\"θησαυρός\"", thes::json_print("θησαυρός"sv)));

  THES_ALWAYS_ASSERT(test::string_eq("true", thes::json_print(true)));

  THES_ALWAYS_ASSERT(
    test::string_eq(R"({ "a": 0, "b": [1, 2] })", thes::json_print(Test4{0, {1, 2}})));
  THES_ALWAYS_ASSERT(test::string_eq("{\n  \"a\": 0,\n  \"b\": [\n    1,\n    2\n  ]\n}",
                                     thes::json_print(Test4{0, {1, 2}}, thes::Indentation{2})));

  THES_ALWAYS_ASSERT(test::string_eq(R"({ "a": 0, "b": { "a": 2, "b": 3 } })",
