...
int main() {
  ...
  THES_ALWAYS_ASSERT(darray1.size() == 3 && test::range_eq(darray1, std::array{0, 0, 0}));

  thes::DynamicArray<int, thes::DefaultInit> darray2(3);
  darray2[2] = 3;
  THES_ALWAYS_ASSERT(darray2.size() == 3 && darray2[2] == 3);

  darray1.resize(9);
  for (const auto i : thes::range<std::size_t>(8)) {
    darray1[i] = static_cast<int>(2 * i + 1);
  }
  THES_ALWAYS_ASSERT(darray1.size() == 9 && darray1.allocation_size() == 16);
  THES_ALWAYS_ASSERT(test::range_eq(darray1, std::array{1, 3, 5, 7, 9, 11, 13, 15, 0}));

  darray2.resize(7);
  for (const auto i : thes::range<std::size_t>(3, 7)) {
    darray2[i] = static_cast<int>(3 * i + 2);
  }
  THES_ALWAYS_ASSERT(darray2.size() == 7 && darray2.allocation_size() == 8);
  THES_ALWAYS_ASSERT(
    test::range_eq(darray2, std::array{darray2[0], darray2[1], 3, 11, 14, 17, 20}));

  for (const int i : thes::range<int>(4)) {
    darray2.push_back(i);
  }
  THES_ALWAYS_ASSERT(darray2.size() == 11 && darray2.allocation_size() == 16);
  THES_ALWAYS_ASSERT(
  ...
  THES_ALWAYS_ASSERT(darray2.size() == 7 && darray2.allocation_size() == 16);
  THES_ALWAYS_ASSERT(
    test::range_eq(darray2, std::array{darray2[0], darray2[1], 3, 11, 14, 17, 20}));

  {
    thes::DynamicArray<S, thes::DefaultInit> darray3(3);
    THES_ALWAYS_ASSERT(darray3.size() == 3 && darray3.allocation_size() == 3);
    THES_ALWAYS_ASSERT(test::range_eq(darray3, std::array{S{}, S{}, S{}}));
    S::counter() = 0;
  }
...

  {
    ...
    THES_ALWAYS_ASSERT(darray4.size() == 3 && darray4.allocation_size() == 3);

    for (const auto i : thes::range<std::size_t>(3)) {
      darray4.initial_emplace(i);
    }
    THES_ALWAYS_ASSERT(darray4.size() == 3 && darray4.allocation_size() == 3);
    THES_ALWAYS_ASSERT(test::range_eq(darray4, std::array{S{}, S{}, S{}}));
    ...
    THES_ALWAYS_ASSERT(S::counter() == 11);
    THES_ALWAYS_ASSERT(darray4.size() == 9 && darray4.allocation_size() == 16);
    THES_ALWAYS_ASSERT(
